<!--
PASTE-READY version of v2 for the LinkedIn article editor. (2026-08-08)

How to use (≈10 minutes):
1. Open this file RENDERED (PyCharm markdown preview, or the file on
   GitHub) — copy from the rendered view, not the raw source, so bold and
   headers survive the paste as rich text.
2. LinkedIn → Write article. Paste the headline into the Title field
   (plain), then copy-paste the body below into the editor. Re-apply the
   header style to any section title the paste demoted to plain text.
3. At each ⟦IMAGE⟧ marker: delete the marker line, insert the image via
   the editor's image button (file paths below), and paste the italic
   caption line into LinkedIn's caption field.
4. Save as draft. Do not publish before the final read.

Image files, in order:
  1. C:\Users\olegr\PycharmProjects\shared-substrate\assets\fig-drift.png
  2. C:\Users\olegr\PycharmProjects\shared-substrate\assets\fig-centroid.png
  3. C:\Users\olegr\PycharmProjects\shared-substrate\assets\fig-projection.png
  4. C:\Users\olegr\PycharmProjects\shared-substrate\assets\fig-loop.png
  5. C:\Users\olegr\PycharmProjects\shared-substrate\assets\fig-stack.png
  6. C:\Users\olegr\PycharmProjects\shared-substrate\article\assets\lyon-canut-revolt.jpg
  7. C:\Users\olegr\PycharmProjects\shared-substrate\article\assets\calculator-protest-1986.jpg

Tags when publishing (max 3): #AI #FutureOfWork #SoftwareEngineering
Note: the Google-comparison table is already converted to bullets below —
LinkedIn articles do not support tables.
-->

**HEADLINE:** The Human Edge Becomes the Job: On Moving Up the Abstraction Stack
...
upon by participants who never met its author. (Philosophy has an old name
for this move — Popper called it World 3: thought given objective
existence, where it can be examined and improved by minds that never met
its maker.) A musical score is the everyday example: not the music and not
the composer, but the composer's intent in durable, addressable notation,
from which any competent orchestra — including one assembled long after the
composer — can expand the sound.
...
- **The shift named.** Google: from writing code to expressing intent.
  This work: from producing representations to engineering intent.
- **The register.** Google: practical — a playbook for teams. This work:
  conceptual — why it works, grounded in cognitive science, information
  theory, and control.
- **The further step.** Google: intent expressed to the system. This work:
  intent **embodied** — durable, addressable, checkable — so it outlives
  the session that expressed it.
...
⟦IMAGE 7 — calculator-protest-1986.jpg⟧
...
## The epoch is new — the job is not

One correction belongs at the end, because it reframes everything above.
Nothing in this essay describes a new human role. The human has been the
creator and engineer of intent from the beginning: every blueprint, every
score, every theory, every ledger was intent given a body so that other
hands — and later, other machines — could carry it. Whitehead observed over
a century ago that civilisation advances by extending the number of
important operations we can perform without thinking of them. Every
generation of tools has done exactly that: absorbed another layer of the
derivable, and left the person holding a purer share of the part that
cannot be derived. The loom did it to the hand, the calculator to
arithmetic, the compiler to machine code. AI is the same movement at a
steeper gradient — the first tool general enough to absorb not one craft's
mechanics but the mechanical layer of nearly all of them at once.

So the paradox of this technology resolves into something almost serene:
**the more the machines absorb of what can be formalised, the more
completely the human is returned to what cannot be.** Aristotle kept
separate words for the two kinds of knowing that our job titles have long
conflated: *techne*, the craft knowledge that can be written down and
taught — and *phronesis*, the judgment that can only be lived: knowing what
should be done, here, for this situation, and why. The machines are finally
mastering techne. Phronesis was never theirs to take. What grows on the
human side of the line is the question behind the work rather than the
work itself: what should exist, what it should mean once it does, how it
serves the whole it belongs to.

And this is not happening to one person at a time. Amplification compounds.
One person freed toward the irreducibly human part of their work is a
career changed; millions of them, coupled to machines that carry the rest,
are a different input into civilisation — an avalanche of released human
capacity, already gathering. Avalanches do not choose their direction;
channels do. Channelled, this is the most constructive force of our
working lifetimes. Unchannelled, it is drift at civilisational scale. That
is what the disciplines of this essay — intent embodied, oracles at every
layer, the governed loop — actually are: not engineering hygiene, but the
channel-works for the largest release of human attention since tools began
absorbing our operations.

The machines are not coming for the human role. They are clearing the way
back to it. The bar is rising, as it always has — and the work on the
other side of it was always ours.
...
Problems" — [doi.org/10.2139/ssrn.7218019](https://doi.org/10.2139/ssrn.7218019)
(preprint), with source and companion materials at
