...
<!--
LinkedIn article draft — v2 (2026-08-08)
Status: DRAFT for review. Supersedes human-edge-becomes-the-job.md (v1).

Changes from v1:
  - New finale, "The epoch is new — the job is not": the continuity frame
    (the human was ALWAYS the creator and engineer of intent; AI is the
    steepest wave of an old unburdening, not a new displacement), the
    paradox stated in calm form, the avalanche + constructive channeling,
    closing on invitation/homecoming rather than the scoreboard.
  - Philosophy dose raised, balanced: Whitehead (operations absorbed) and
    Aristotle (techne vs phronesis) in the finale, Popper (World 3) as a
    parenthetical in "Intent, embodied" + the musical-score image from the
    paper. Each named reference is one sentence and glossed in plain words.
  - Article now ends at the altitude it aims at: person → organisation →
    civilisation, closing on the oldest job, finally unencumbered.
