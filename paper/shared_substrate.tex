...
mass of the coupled human--AI system, held stationary by
...
% pinned centroid
\filldraw[fill=blue!25,draw=blue!60,thick] (0,0) circle (0.46);
\node[font=\scriptsize,align=center] at (0,0) {shared\\substrate};
% pin: post + ground hatching
\draw[thick,blue!60] (0,-0.46) -- (0,-0.95);
\draw[thick,blue!60] (-0.42,-0.95) -- (0.42,-0.95);
\foreach \x in {-0.42,-0.24,-0.06,0.12,0.30}
  \draw[blue!60] (\x,-0.95) -- (\x+0.14,-1.12);
...
% excursion (drift): agent drifting outward
\draw[->,>=stealth,red!70,thick] (-2.55,0.85) -- (-3.30,1.55);
\node[font=\scriptsize,red!75,anchor=south east] at (-2.55,1.50) {drift};
% reversion (warm-up): human arrow bending to centroid
\draw[->,>=stealth,blue!70,thick] (2.55,0.85) to[bend left=18] (0.55,0.12);
\node[font=\scriptsize,blue!75,anchor=south] at (1.80,0.15) {warm-up};
\end{tikzpicture}
\caption{The pinned centroid. Participants in sustained work ---
human, agents, successive models, colleagues --- are noisy point
masses. The substrate is the deliberately pinned centre of mass of
the ensemble: drift is excursion away from it, warm-up is
reversion to it, and a model swap moves a point mass without
moving the centroid.}
\label{fig:centroid}
\end{figure}
...
failure in \citet{becker2025metr} --- practitioners who are slowed
down while confidently believing they are sped up --- means felt
productivity cannot be trusted as the measure of a human--AI
coupling; only externalised, inspectable state can be. Second, the
...
What Shared Substrate asks of a practitioner is not foreign to
...
file as its session entry point. Practitioners cultivating a
...
every step performed by the practitioner. \S\ref{sec:governance}
...
\draw[sig] (agent) -- (work);
% feedback path
\node[blk,fill=orange!14,minimum width=4.8cm] (obs) at (2.9,-1.65)
...
% expansion cone
\fill[blue!5] (-1.8,1.1) -- (2.2,2.0) -- (2.2,-2.1) -- (-1.8,-1.1) -- cycle;
\draw[blue!40] (-1.8,1.1) -- (2.2,2.0);
\draw[blue!40] (-1.8,-1.1) -- (2.2,-2.1);
...
\draw[sig,dashed,blue!60] (3.0,-2.1)
  .. controls (1.0,-2.85) and (-0.8,-2.5) .. (-1.95,-1.2);
...
they are the incompressible kernel. This gives the discipline's
...
compression reading above is qualitative. A formal treatment
...
specified, every step is performed by the practitioner; left
...
execute it on the practitioner's behalf.
...
looks like in flight. Both are single-practitioner accounts,
...
the discipline from the contribution of the practitioner applying
...
A practitioner attempting Shared Substrate for the first time
...
Substrate helps a practitioner who has something to say organise
...
\section{The Practitioner's Compact Field Manual}
\label{app:manual}
