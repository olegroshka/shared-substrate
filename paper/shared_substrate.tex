...
% handoff (deposit): a session's results written back into the centroid
\draw[->,>=stealth,blue!70,thick] (2.55,0.85) to[bend right=24] (0.55,-0.45);
\node[font=\scriptsize,blue!75,anchor=north] at (2.10,-0.32) {handoff};
\end{tikzpicture}
...
reversion to it, handoff deposits a session's results into it, and
a model swap moves a point mass without moving the centroid. The
pin is authorship, and it fixes the centroid not to a place but to
the course of the work: what stays constant is reference, not
position.}
\label{fig:centroid}
\end{figure}
