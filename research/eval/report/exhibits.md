...
|----|---|---|---|---|
| A1 | **Retransmission tax.** "Re-entry costs 4× as much, is paid half as often, and moves the wrong way in the ephemeral project." *(caption corrected S9: the bench said "twice as often"; the data and REPORT §2a say the ephemeral project warms up half as often — prefix 0.53 vs 1.00 — at 4× the cost)* | blive warms up in 9/10 sessions at ~106 chars, **falling** 192→106; btest in 43/68 at ~417, **rising** 477→607. Corpus: 78 sessions, 1,061 classifiable turns | `data/session-metrics/warmup.json` | btest's sessions also lengthened (9.1→19.8 turns); paste bodies survive 74/204, so payload-shaped warm-ups are under-counted — conservative against the finding |
